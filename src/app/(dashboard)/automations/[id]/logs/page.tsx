    ...
    <div className="space-y-6">
      <div className="isolate flex items-center gap-3">
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-surface-sunken hover:text-foreground"
          aria-label="Back"
        >
        ...
        <ul className="space-y-2">
          {logs.map((log) => {
            ...
            return (
              <li
                key={log.id}
                className="rounded-xl   bg-card backdrop-blur-[var(--blur-glass)]"
              >
                <button
                  type="button"
                ...
                {isOpen && (
                  <div className="  px-4 py-3">
                    {log.error_message && (
                      <p className="mb-3 rounded-md  border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-red-300">
                        {log.error_message}
                      </p>
                    )}
