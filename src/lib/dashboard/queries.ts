...
export async function loadMetrics(db: DB, accountId?: string): Promise<MetricsBundle> {
  ...
  let conversations = db.from('conversations').select('id', { count: 'exact', head: true }).eq('status', 'open')
  let convToday = db.from('conversations').select('id', { count: 'exact', head: true }).eq('status', 'open').gte('created_at', todayStart)
  let convYesterday = db.from('conversations').select('id', { count: 'exact', head: true }).eq('status', 'open').gte('created_at', yesterdayStart).lt('created_at', todayStart)
  ...
  if (accountId) {
    ...
    convToday = convToday.eq('account_id', accountId)
    convYesterday = convYesterday.eq('account_id', accountId)
    contactsToday = contactsToday.eq('account_id', accountId)
    contactsYesterday = contactsYesterday.eq('account_id', accountId)
    deals = deals.eq('account_id', accountId)
    // messages doesn't have account_id, but it's joined to conversations
    // which has account_id. For now, messages relies on RLS or we'd need
    // to join. Given the task is Deals remediation, I'll stick to deals.
  }
...
export async function loadPipelineDonut(db: DB, accountId?: string): Promise<PipelineDonutData> {
  ...
  if (accountId) {
    // pipeline_stages doesn't have account_id directly, it's joined to pipelines.
    // However, migration 017 added account_id to pipelines, and RLS should
    // handle pipeline_stages. For robustness, we could join, but pipelines
    // themselves are already scoped.
    // Actually, deals has account_id directly.
    dealsQuery = dealsQuery.eq('account_id', accountId)
  }
...
export async function loadActivity(db: DB, accountId?: string, limit = 20): Promise<ActivityItem[]> {
  ...
  if (accountId) {
    ...
    dealsQuery = dealsQuery.eq('account_id', accountId)
    broadcastsQuery = broadcastsQuery.eq('account_id', accountId)
    autoLogsQuery = autoLogsQuery.eq('account_id', accountId)
  }
