...
export function ConversationsChart({ series, loading, range, onRangeChange }: ConversationsChartProps) {
  ...
  return (
    <section className="flex h-full flex-col rounded-2xl   bg-card shadow-raised backdrop-blur-[var(--blur-glass)]">
      <header className="flex items-center justify-between px-5 py-4">
        ...
        <div className="flex items-center gap-1 rounded-lg bg-surface-sunken p-1 shadow-inset-sm">
          {[7, 30, 90].map((r) => (
            <button
              key={r}
              type="button"
      ...
      <footer className="flex items-center gap-4 px-5 py-3 text-xs text-muted-foreground">
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      ...
      >
        ...
        {/* Outgoing polyline (violet, brand primary) */}
        <path
          d={outgoingPath}
          fill="none"
          stroke="var(--primary)"
        ...
        />
        {/* Incoming polyline (cyan, brand secondary) */}
        <path
          d={incomingPath}
          fill="none"
          stroke="var(--chart-2)"
      ...
      {hovered && hover !== null && (
        <div
          className="pointer-events-none absolute top-0 z-10 -translate-x-1/2 rounded-lg   bg-popover px-2.5 py-1.5 text-[11px] shadow-floating backdrop-blur-[var(--blur-glass-strong)]"
        ...
        >
          ...
          <div className="mt-1 flex flex-col gap-0.5">
            <span className="flex items-center gap-1.5" style={{ color: 'var(--chart-2)' }}>
              <span className="inline-block h-1.5 w-1.5 rounded-full" style={{ background: 'var(--chart-2)' }} />
