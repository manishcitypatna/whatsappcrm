...
export function ActivityFeed({ items, loading }: ActivityFeedProps) {
  ...
  return (
    <section className="rounded-2xl bg-card shadow-raised">
      <header className="flex items-center justify-between px-5 py-4 shadow-[inset_0_-2px_5px_-2px_var(--shadow-tint)]">
        ...
        <>
          <ul className=" ">
            {visible.map((it, i) => {
              ...
              const stripe = i % 2 === 0 ? 'bg-transparent' : 'bg-surface-sunken/40'
              const row = (
              ...
              return (
                <li key={it.id} className={cn(stripe, 'transition-colors hover:bg-surface-sunken/60')}>
                  {it.href ? (
          ...
          <footer className="flex items-center justify-between px-5 py-3 text-xs shadow-[inset_0_2px_5px_-2px_var(--shadow-tint)]">
