...
export function ValidationPanel() {
  ...
  if (issues.length === 0) {
    ...
    return (
      <div className="flex items-center gap-2 rounded-lg  border-emerald-600/50 bg-background p-3 text-sm font-medium text-emerald-300">
  ...
  const body = (
    <>
      ...
      <span className="min-w-0 flex-1">
        {issue.node_key && (
          <code className="mr-1 rounded bg-surface-sunken px-1 py-0.5 text-[10px] text-muted-foreground">
            {issue.node_key}
          </code>
        )}
        {issue.message}
  ...
  if (issue.node_key && onJump) {
    return (
      <button
        ...
        className={cn(
          "flex w-full items-start gap-2 rounded-md px-2 py-1 text-left text-xs transition-colors hover:bg-surface-sunken/60",
          tone,
        )}
