...
interface NodeData extends Record<string, unknown> {
  node: BuilderNode;
  isEntry: boolean;
  /** Validator's "look here" pulse — flashes the card  for
...
function FlowNodeCard({ data, selected }: NodeProps) {
  ...
  return (
    <div
      className={cn(
        "relative min-w-[220px] max-w-[260px] rounded-lg bg-card/95 px-3 py-2 text-left shadow-raised backdrop-blur-[var(--blur-glass)] transition-all",
        selected
        ...
        isFlashed && "ring-2 ring-amber-400/60",
      )}
    >
      {hasTarget && (
        <Handle
          ...
          className="!h-2.5 !w-2.5 ! !bg-surface-sunken"
        />
      )}
          ...
          {slots.map((slot) => (
            ...
            >
              ...
              <Handle
                ...
                className="!relative !right-auto !top-auto !h-2.5 !w-2.5 !translate-x-[12px] !transform-none ! !bg-surface-sunken"
              />
            </div>
          ))}
        </div>
...
function FlowCanvasInner() {
  ...
  if (rfNodes.length === 0) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-3 rounded-lg    bg-background text-sm text-muted-foreground">
        <p>No nodes yet.</p>
        <CanvasAddNodeButton />
      </div>
    );
  ...
  return (
    <>
      <div className="h-[70vh] w-full overflow-hidden rounded-lg   bg-background">
        <ReactFlow
          nodes={rfNodes}
          edges={rfEdges}
        ...
        >
          ...
          <Controls
            className="! !bg-card backdrop-blur-[var(--blur-glass)] [&_button]:! [&_button]:!bg-card backdrop-blur-[var(--blur-glass)] [&_button:hover]:!bg-surface-sunken"
          ...
          <MiniMap
            ...
            className="! ! !bg-card backdrop-blur-[var(--blur-glass)]"
          />
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      ...
      >
        ...
        <SheetFooter className="  px-5 py-3 sm:flex-row sm:justify-between">
          {!isEntry ? (
...
function CanvasAddNodeButton() {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        className="inline-flex items-center gap-1.5 rounded-md   bg-card backdrop-blur-[var(--blur-glass)] px-3 py-1.5 text-xs font-medium text-foreground shadow-lg transition-colors hover:bg-surface-sunken"
        aria-label="Add node"
      >
        <Plus className="h-3.5 w-3.5" />
        Add node
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className=" bg-card backdrop-blur-[var(--blur-glass)]">
