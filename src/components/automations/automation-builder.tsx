...
// ------------------------------------------------------------
// Step metadata — one source of truth for icon + label +  color
// ------------------------------------------------------------

interface StepMeta {
  label: string
  icon: typeof Zap
  /** Icon-badge tint per spec — replaces the old left-border accent
   *  (borderless glass design: color-coding lives in a tint, not an
   *  outline). */
  tint: string
}

const STEP_META: Record<AutomationStepType, StepMeta> = {
  send_message: { label: "Send Message", icon: MessageSquare, tint: "bg-primary/15 text-primary" },
  send_template: { label: "Send Template", icon: FileText, tint: "bg-primary/15 text-primary" },
  add_tag: { label: "Add Tag", icon: Tag, tint: "bg-primary/15 text-primary" },
  remove_tag: { label: "Remove Tag", icon: TagIcon, tint: "bg-primary/15 text-primary" },
  assign_conversation: { label: "Assign Conversation", icon: UserCheck, tint: "bg-primary/15 text-primary" },
  update_contact_field: { label: "Update Contact Field", icon: PencilLine, tint: "bg-primary/15 text-primary" },
  create_deal: { label: "Create Deal", icon: Briefcase, tint: "bg-primary/15 text-primary" },
  wait: { label: "Wait", icon: Hourglass, tint: "bg-surface-sunken text-muted-foreground" },
  condition: { label: "Condition (If/Else)", icon: GitBranch, tint: "bg-amber-500/15 text-amber-500" },
  send_webhook: { label: "Send Webhook", icon: Webhook, tint: "bg-primary/15 text-primary" },
  close_conversation: { label: "Close Conversation", icon: CircleSlash, tint: "bg-primary/15 text-primary" },
}
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-background">
      {/* Top bar. At sub-sm widths the "Active" label is hidden and the
          ...
          gets maximum width. */}
      <header className="flex flex-shrink-0 items-center gap-2   bg-card/80 px-3 py-3 sm:gap-3 sm:px-4">
        <button
          type="button"
          onClick={() => router.push("/automations")}
          className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-surface-sunken hover:text-foreground"
        ...
        <input
          ...
          className="min-w-0 flex-1 rounded-md bg-transparent px-2 py-1 text-sm font-semibold text-foreground placeholder:text-muted-foreground focus:bg-surface-sunken focus:outline-none sm:text-base"
        />
...
function TriggerCard({
  ...
  return (
    ...
    <div className="z-10 w-full max-w-[320px] sm:w-80">
      <div className="rounded-lg bg-card backdrop-blur-[var(--blur-glass)] shadow-lg">
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          ...
          <div className="space-y-3   px-4 py-3">
            <div>
              ...
              <select
                ...
                className="w-full rounded-none border-b-2 border-border bg-transparent px-2 py-1.5 text-sm text-foreground shadow-none focus:outline-none focus:border-primary"
              >
    ...
    <div className="space-y-2">
      ...
      <div>
        ...
        <select
          ...
          className="w-full rounded-none border-b-2 border-border bg-transparent px-2 py-1.5 text-sm text-foreground focus:outline-none focus:border-primary"
        >
      ...
      <div className={cn("z-10 flex flex-col", width)}>
        <div className="rounded-lg bg-card backdrop-blur-[var(--blur-glass)] shadow-lg">
          ...
          >
            ...
            <div className={cn("flex h-8 w-8 items-center justify-center rounded-md", meta.tint)}>
...
function AddButton({ onPick }: { onPick: (t: AutomationStepType) => void }) {
  return (
    <div className="relative flex flex-col items-center">
      <div className="h-4 w-[2px] bg-surface-sunken" aria-hidden />
      <DropdownMenu>
        <DropdownMenuTrigger
          className="flex h-8 w-8 items-center justify-center rounded-full bg-background shadow-inset-sm text-muted-foreground transition-all hover:bg-primary/10 hover:text-primary hover:shadow-raised-sm data-[popup-open]:bg-primary/20 data-[popup-open]:text-primary data-[popup-open]:shadow-inset"
          aria-label="Add step"
        >
        ...
        <DropdownMenuContent
          align="start"
          className="max-h-80 min-w-56 overflow-y-auto  bg-card backdrop-blur-[var(--blur-glass)]"
        >
          {ADDABLE_STEPS.map((t) => {
